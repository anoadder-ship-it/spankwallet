...
describe("spankwallet: PendingAction - initiate/finalize/cancel voor alle vier kinds (STATUS.md sectie 124)", function () {
  ...
  describe("kind=0 SolWithdrawal (initiate_withdrawal/finalize_withdrawal)", () => {
    ...
    it("5. recovery tijdens pending: initiate_recovery sluit de wachtende actie; na een voltooide recovery is er niets meer af te ronden (sectie 160; was: PendingActionStaleEpoch)", async () => {
      ...
      assert.isNull(
        await provider.connection.getAccountInfo(pendingActionPda),
        "sectie 160: initiate_recovery had de wachtende actie moeten sluiten"
      );
      ...
      // Sectie 160: initiate_recovery sloot de actie al; na de recovery is er
      // niets meer om af te ronden. De epoch-check (PendingActionStaleEpoch)
      // is daardoor via de publieke instructies niet meer te bereiken en
      // blijft staan als tweede verdedigingslaag.
      assert.isNull(await provider.connection.getAccountInfo(pendingActionPda));
    ...
    // Sectie 160: omgedraaid. Vóór sectie 160 overleefde een wachtende
    // actie initiate_recovery en annuleerde cancel_action die tijdens de
    // recovery. Nu sluit initiate_recovery de actie zelf (rent naar de
    // backup authority); cancel_action blijft tijdens een recovery
    // toegestaan, maar er is niets meer te annuleren.
    it("8b. tijdens een lopende recovery is de wachtrij leeg: initiate_recovery sloot de actie, cancel_action heeft niets te annuleren (NoPendingAction) (sectie 160)", async () => {
      const recoveryTimelockSeconds = 3;
      ...
      assert.isNull(
        await provider.connection.getAccountInfo(pendingActionPda),
        "initiate_recovery had de wachtende actie moeten sluiten"
      );

      await expectAnchorError(
        callCancelAction(passkey, walletPda, pendingActionPda, passkeysPda),
        "NoPendingAction"
      );
    });
  ...
  describe("sessie-geïnitieerde AdvancedAction (initiate_advanced_action_via_session / confirm_pending_action) - STATUS.md sectie 153", () => {
    ...
    it("confirm_pending_action na een voltooide recovery: initiate_recovery sloot de sessie-actie al, er valt niets meer te bevestigen (sectie 160; was: PendingActionStaleEpoch)", async () => {
      ...
      assert.isNull(
        await provider.connection.getAccountInfo(wallet.pendingActionPda),
        "sectie 160: initiate_recovery had de wachtende actie moeten sluiten"
      );
    ...
    // Sectie 160: omgedraaid. Vóór sectie 160 bleef het slot tijdens een
    // recovery bezet en maakte cancel_action het vrij; nu sluit
    // initiate_recovery de sessie-actie zelf.
    it("BEZET SLOT tijdens een lopende recovery: initiate_recovery maakt het slot vrij, en de sessie kan dan niet opnieuw initiëren (sectie 160)", async () => {
      const wallet = await createWallet();
      await callAddAllowedProgram(wallet.passkey, wallet.walletPda, wallet.policyPda, SystemProgram.programId);
      const sessionKeypair = Keypair.generate();
      await callAddAdvancedSessionKey(
  ...
  /// WalletAccount.recovery_nonce_snapshot (sectie 159), rechtstreeks uit de
  /// ruwe accountbytes: het veld staat achter `disarmed`, dus direct na
  /// action_nonce(8) + session_epoch(8) + spend_threshold_lamports(8) +
  /// disarmed(1). Alleen betekenisvol zolang recovery_state Some is.
  async function recoveryNonceSnapshotProcessed(walletPda: PublicKey): Promise<bigint> {
    const info = await provider.connection.getAccountInfo(walletPda, "processed");
    return info!.data.readBigUInt64LE(actionNonceOffset(info!.data) + 8 + 8 + 8 + 1);
  }

  interface CancelRecoveryFields {
    nonceSnapshot: bigint;
    initiatedAt: bigint;
    newOwnerPasskey: Buffer;
  }

  /// [secp256r1-instructie, cancel_recovery] voor precies deze recovery.
  /// Sectie 159: challenge cancel_recovery_v3 over
  /// recovery_nonce_snapshot || initiated_at || new_owner_passkey, zonder de
  /// live nonce. Met de hand opgebouwd (zie instructionDiscriminator).
  function buildCancelRecoveryIxsFor(
    signingPasskey: TestPasskey,
    walletPda: PublicKey,
    passkeysPda: PublicKey,
    fields: CancelRecoveryFields
  ) {
    const initiatedAt = Buffer.alloc(8);
    initiatedAt.writeBigInt64LE(fields.initiatedAt, 0);
    const payload = Buffer.concat([nonceLeBytes(fields.nonceSnapshot), initiatedAt, fields.newOwnerPasskey]);
    const expectedChallenge = buildExpectedChallenge(program.programId, walletPda, "cancel_recovery_v3", payload);
  ...
  /// Zelfde als hierboven, met de velden van de lopende recovery van de keten.
  async function buildCancelRecoveryIxs(signingPasskey: TestPasskey, walletPda: PublicKey, passkeysPda: PublicKey) {
    ...
    return buildCancelRecoveryIxsFor(signingPasskey, walletPda, passkeysPda, {
      nonceSnapshot: await recoveryNonceSnapshotProcessed(walletPda),
      initiatedAt: BigInt(recovery.initiatedAt.toString()),
      newOwnerPasskey: Buffer.from(recovery.newOwnerPasskey),
    });
  }
  ...
  async function outcomeOf(promise: Promise<unknown>): Promise<string> {
    try {
      ...
      // Sectie 160: een transactie die de simulatie haalt maar on-chain
      // faalt (bijv. de seconde slaat om tussen simulatie en uitvoering),
      // komt terug als {"Custom":N} zonder logs; vertaal via de IDL.
      const custom = /"Custom":(\d+)/.exec(text);
      if (custom) {
        const errors = (program.idl.errors ?? []) as Array<{ code: number; name: string }>;
        const entry = errors.find((e) => e.code === Number(custom[1]));
        if (entry) {
  ...
  describe("Recovery: unfreeze via de backup authority weigert tijdens een lopende recovery; bevriezen blijft toegestaan (zie sectie 159); cancel_recovery is gebonden aan de recovery (sectie 158)", () => {
      ...
      assert.deepEqual(
        {
          ...
          WalletDisarmed: idlErrorNumber("WalletDisarmed"),
          NoPendingAction: idlErrorNumber("NoPendingAction"),
          WalletNotDisarmed: idlErrorNumber("WalletNotDisarmed"),
        ...
        },
        {
          ...
          WalletNotDisarmed: 6067,
          WalletAlreadyDisarmed: 6068,
        }
      );
    });

    // Sectie 159: omgedraaid. Vóór sectie 159 weigerde bevriezen via de
    // backup authority tijdens een recovery; nu slaagt het.
    it("[158/159] bevriezen via de backup authority slaagt tijdens een lopende recovery; de recovery blijft lopen, de passkey-set blijft gelijk, en een eerder ondertekende cancel_recovery landt daarna", async () => {
      const w = await createWallet();
      await callInitiateRecovery(w.backupAuthority, w.walletPda, dummyNewOwnerPasskey());
      ...
      const nonceBump = Number((await actionNonceProcessed(w.walletPda)) - nonceBefore);
      const passkeysUnchanged = snapshotsEqual(passkeysBefore, await snapshot([w.passkeysPda]));
      const recoveryStillActive = (await program.account.walletAccount.fetch(w.walletPda, "processed")).recoveryState !== null;
    ...
    // Sectie 159: omgedraaid. Bevriezen via de backup authority slaagt nu
    // tijdens een recovery (idempotent, elke keer +1 nonce); direct ontdooien
    // blijft weigeren. De eerste poging is een ontdooiing op een NIET-bevroren
    // wallet: bewijst dat RecoveryAlreadyInProgress vóór WalletNotDisarmed komt.
    it("[158/159] herhaalde backup-pogingen tijdens een lopende recovery: bevriezen slaagt (idempotent), direct ontdooien weigert (RecoveryAlreadyInProgress, ook op een niet-bevroren wallet); passkey-set en wachtrij ongewijzigd; een vóór de pogingen ondertekende cancel_recovery landt, en de recovery wordt daarna niet afgerond (NoRecoveryInProgress)", async () => {
      const w = await createWallet(3);
      await callInitiateRecovery(w.backupAuthority, w.walletPda, dummyNewOwnerPasskey());
      ...
      const nonceBefore = await actionNonceProcessed(w.walletPda);
      const before = await snapshot([w.passkeysPda, w.pendingActionPda]);
      ...
      const nonceBumps = Number((await actionNonceProcessed(w.walletPda)) - nonceBefore);
      const passkeysAndQueueUnchanged = snapshotsEqual(before, await snapshot([w.passkeysPda, w.pendingActionPda]));
      const afterAttempts = await program.account.walletAccount.fetch(w.walletPda, "processed");
      const cancel = await outcomeOf(provider.sendAndConfirm(new anchor.web3.Transaction().add(...cancelIxs)));
      ...
      const blockedUnfreeze = expectedError("RecoveryAlreadyInProgress");
      assert.deepEqual(
        {
          attempts,
          nonceBumps,
          passkeysAndQueueUnchanged,
          recoveryActiveAfterAttempts: afterAttempts.recoveryState !== null,
          disarmedAfterAttempts: afterAttempts.disarmed,
          cancel,
          finalize,
          ownerPasskeyUnchanged: Buffer.from(wallet.ownerPasskey).equals(w.passkey.compressedPublicKey),
        },
        {
          attempts: [blockedUnfreeze, "ok", blockedUnfreeze, "ok", blockedUnfreeze, "ok", blockedUnfreeze],
          nonceBumps: 3,
          passkeysAndQueueUnchanged: true,
          recoveryActiveAfterAttempts: true,
          disarmedAfterAttempts: true,
          cancel: "ok",
    ...
    it("[158] cancel_recovery is gebonden aan de recovery zelf: een eerder ondertekende cancel landt ook na tussentijdse nonce-ophogingen, en werkt niet tegen een andere recovery (WebAuthnChallengeMismatch)", async () => {
      ...
      const second = generateTestPasskey();
      await callAddPasskey(w.passkey, w.walletPda, w.passkeysPda, second.compressedPublicKey);
      // Sectie 160: tweede nonce-ophoging tijdens de recovery is nu bevriezen
      // via de backup authority (sectie 159) i.p.v. cancel_action - een
      // wachtende actie overleeft initiate_recovery niet meer.
      const newOwner = dummyNewOwnerPasskey();
      ...
      const freezeViaBackup = await outcomeOf(callFreezeViaBackup(w.backupAuthority, w.walletPda));
      ...
      assert.deepEqual(
        { freeze, freezeViaBackup, bumps, first, replayOtherKey, replayLaterStart },
        {
          freeze: "ok",
          freezeViaBackup: "ok",
          bumps: 2,
  ...
  // ================= Recovery: bevriezen tijdens een recovery, momentopname van de nonce (sectie 159) =================
  ...
  async function buildExecuteIxs(
    ...
    const { secp256r1Ix, clientDataJSON } = await passkeyIxs(
      ...
      walletPda,
      "execute",
      Buffer.concat([recipient.toBuffer(), amount.toArrayLike(Buffer, "le", 8)]),
      nonce
    );
    const [spendWindowPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("spend_window"), walletPda.toBuffer()],
    ...
    const executeIx = await program.methods
      ...
      .instruction();
    return [secp256r1Ix, executeIx];
  }

  /// Clock::unix_timestamp zoals het programma die ziet (Clock-sysvar, offset 32).
  async function clockUnixTimestampProcessed(): Promise<bigint> {
    const info = await provider.connection.getAccountInfo(SYSVAR_CLOCK_PUBKEY, "processed");
    return info!.data.readBigInt64LE(32);
  }

  async function lamportsProcessed(address: PublicKey): Promise<number> {
    return provider.connection.getBalance(address, "processed");
  }

  describe("Recovery: bevriezen via de backup authority tijdens een recovery; cancel_recovery gebonden aan een momentopname van de nonce (sectie 159)", () => {
    it("[159] M1: de eigenaar heeft alleen nog de backup-sleutel en bevriest tijdens de recovery; een tweede passkey-houder die annuleert en in dezelfde transactie uitgeeft, faalt (WalletDisarmed)", async () => {
      const w = await createWallet();
      const other = generateTestPasskey();
      await callAddPasskey(w.passkey, w.walletPda, w.passkeysPda, other.compressedPublicKey);
      await callInitiateRecovery(w.backupAuthority, w.walletPda, dummyNewOwnerPasskey());
      ...
      const vaultBefore = await lamportsProcessed(w.vaultPda);
      const nonce = await actionNonceProcessed(w.walletPda);
      const cancelIxs = await buildCancelRecoveryIxs(other, w.walletPda, w.passkeysPda);
      // cancel_recovery verhoogt de nonce met 1; de uitgave is daarop ondertekend.
      const executeIxs = await buildExecuteIxs(other, w.walletPda, w.vaultPda, w.passkeysPda, recipient, amount, nonce + 1n);
      const cancelAndSpend = await outcomeOf(
        provider.sendAndConfirm(new anchor.web3.Transaction().add(...cancelIxs, ...executeIxs))
      );
      const vaultUnchanged = (await lamportsProcessed(w.vaultPda)) === vaultBefore;
      const recipientBalance = await lamportsProcessed(recipient);
      const wallet = await program.account.walletAccount.fetch(w.walletPda, "processed");
      ...
      assert.deepEqual(
        {
          ...
          vaultUnchanged,
          recipientBalance,
          recoveryStillActive: wallet.recoveryState !== null,
          disarmed: wallet.disarmed,
        },
        {
          freeze: "ok",
          cancelAndSpend: expectedError("WalletDisarmed"),
          vaultUnchanged: true,
          recipientBalance: 0,
          recoveryStillActive: true,
          disarmed: true,
    ...
    it("[159] L1: twee recovery's in dezelfde seconde met dezelfde nieuwe sleutel; de publieke cancel-handtekening van de eerste werkt niet tegen de tweede (WebAuthnChallengeMismatch)", async () => {
      const w = await createWallet();
      const newOwner = dummyNewOwnerPasskey();
      const initiateIx = await program.methods
        .initiateRecovery(newOwner)
        .accounts({ wallet: w.walletPda, backupAuthority: w.backupAuthority.publicKey })
        .instruction();

      // [initiate(K), cancel(S), initiate(K)] in één transactie: beide recovery's
      // hebben dan per definitie hetzelfde initiated_at. S wordt vooraf
      // ondertekend over de voorspelde klokwaarde; is de seconde intussen
      // omgeslagen, dan faalt de hele transactie en volgt een nieuwe poging.
      let landed: { cancelIxs: TransactionInstruction[]; predictedAt: bigint; nonceAtFirst: bigint } | null = null;
      for (let attempt = 0; attempt < 20 && landed === null; attempt++) {
        const predictedAt = await clockUnixTimestampProcessed();
        const nonceAtFirst = await actionNonceProcessed(w.walletPda);
        const cancelIxs = buildCancelRecoveryIxsFor(w.passkey, w.walletPda, w.passkeysPda, {
          nonceSnapshot: nonceAtFirst,
          initiatedAt: predictedAt,
          newOwnerPasskey: Buffer.from(newOwner),
        });
        const outcome = await outcomeOf(
          sendWithBackup(w.backupAuthority, uniqueIx(attempt), initiateIx, ...cancelIxs, initiateIx)
        );
        if (outcome === "ok") {
          landed = { cancelIxs, predictedAt, nonceAtFirst };
        } else if (outcome !== expectedError("WebAuthnChallengeMismatch")) {
          throw new Error(`onverwachte uitkomst van de gecombineerde transactie: ${outcome}`);
        }
      }
      assert.isNotNull(landed, "de gecombineerde transactie landde niet binnen 20 pogingen");

      const second = (await program.account.walletAccount.fetch(w.walletPda, "processed")).recoveryState!;
      const secondSnapshot = await recoveryNonceSnapshotProcessed(w.walletPda);
      const replay = await outcomeOf(provider.sendAndConfirm(new anchor.web3.Transaction().add(...landed!.cancelIxs)));

      assert.deepEqual(
        {
          sameSecond: BigInt(second.initiatedAt.toString()) === landed!.predictedAt,
          sameNewOwner: Buffer.from(second.newOwnerPasskey).equals(Buffer.from(newOwner)),
          secondSnapshot: secondSnapshot.toString(),
          replay,
        },
        {
          sameSecond: true,
          sameNewOwner: true,
          secondSnapshot: (landed!.nonceAtFirst + 1n).toString(),
          replay: expectedError("WebAuthnChallengeMismatch"),
        }
      );
    });

    it("[159] volgordes in één transactie: bevriezen via backup + cancel_recovery, initiate_recovery + bevriezen via backup, bevriezen via backup + initiate_recovery slagen; initiate_recovery + direct ontdooien weigert atomisch (RecoveryAlreadyInProgress)", async () => {
      // Bevriezen via de backup authority, dan cancel_recovery, tijdens een recovery.
      const a = await createWallet();
      await callInitiateRecovery(a.backupAuthority, a.walletPda, dummyNewOwnerPasskey());
      const freezeThenCancel = await outcomeOf(
        sendWithBackup(
          a.backupAuthority,
          await buildFreezeViaBackupIx(a.backupAuthority, a.walletPda),
          ...(await buildCancelRecoveryIxs(a.passkey, a.walletPda, a.passkeysPda))
      ...
      // initiate_recovery, dan bevriezen via de backup authority.
      const b = await createWallet();
      const initiateThenFreeze = await outcomeOf(
        sendWithBackup(
          b.backupAuthority,
          await program.methods
            .initiateRecovery(dummyNewOwnerPasskey())
            .accounts({ wallet: b.walletPda, backupAuthority: b.backupAuthority.publicKey })
            .instruction(),
          await buildFreezeViaBackupIx(b.backupAuthority, b.walletPda)
        )
      );
      const bWallet = await program.account.walletAccount.fetch(b.walletPda, "processed");

      // Bevriezen via de backup authority, dan initiate_recovery.
      const c = await createWallet();
      const freezeThenInitiate = await outcomeOf(
        sendWithBackup(
          c.backupAuthority,
          await buildFreezeViaBackupIx(c.backupAuthority, c.walletPda),
          await program.methods
            .initiateRecovery(dummyNewOwnerPasskey())
            .accounts({ wallet: c.walletPda, backupAuthority: c.backupAuthority.publicKey })
            .instruction()
        )
      );
      const cWallet = await program.account.walletAccount.fetch(c.walletPda, "processed");

      // initiate_recovery, dan direct ontdooien, op een bevroren wallet.
      const d = await createWallet();
      await callFreezeViaPasskey(d.passkey, d.walletPda, d.passkeysPda);
      const dBefore = await snapshot([d.walletPda, d.passkeysPda, d.pendingActionPda]);
      const initiateThenUnfreeze = await outcomeOf(
        sendWithBackup(
          d.backupAuthority,
          await program.methods
            .initiateRecovery(dummyNewOwnerPasskey())
            .accounts({ wallet: d.walletPda, backupAuthority: d.backupAuthority.publicKey })
            .instruction(),
          buildUnfreezeViaBackupIx(d.backupAuthority, d.walletPda, d.pendingActionPda, d.passkeysPda)
        )
      );
      const dUnchanged = snapshotsEqual(dBefore, await snapshot([d.walletPda, d.passkeysPda, d.pendingActionPda]));
      ...
      assert.deepEqual(
        {
          ...
          aDisarmed: aWallet.disarmed,
          initiateThenFreeze,
          bRecoveryActive: bWallet.recoveryState !== null,
          bDisarmed: bWallet.disarmed,
          freezeThenInitiate,
          cRecoveryActive: cWallet.recoveryState !== null,
          cDisarmed: cWallet.disarmed,
          initiateThenUnfreeze,
          dUnchanged,
        },
        {
          freezeThenCancel: "ok",
          aRecoveryCancelled: true,
          aDisarmed: true,
          initiateThenFreeze: "ok",
          bRecoveryActive: true,
          bDisarmed: true,
          freezeThenInitiate: "ok",
          cRecoveryActive: true,
          cDisarmed: true,
          initiateThenUnfreeze: expectedError("RecoveryAlreadyInProgress"),
          dUnchanged: true,
        }
      );
    });

    it("[159] de momentopname is de nonce bij initiate_recovery; finalize_recovery verhoogt de nonce en zet de momentopname op 0, zodat een volgende recovery altijd een hogere momentopname heeft", async () => {
      const w = await createWallet(3);
      // Eén passkey-actie vooraf, zodat de nonce bij initiate_recovery niet 0 is.
      await callAddPasskey(w.passkey, w.walletPda, w.passkeysPda, generateTestPasskey().compressedPublicKey);
      const newOwner = dummyNewOwnerPasskey();

      const nonceAtFirst = await actionNonceProcessed(w.walletPda);
      await callInitiateRecovery(w.backupAuthority, w.walletPda, newOwner);
      const firstSnapshot = await recoveryNonceSnapshotProcessed(w.walletPda);
      const initiatedAt = (await program.account.walletAccount.fetch(w.walletPda, "processed")).recoveryState!.initiatedAt;

      await advanceOnChainClockPast(provider.connection, payerKeypair(), initiatedAt.toNumber() + 3);
      await callFinalizeRecovery(w.walletPda, w.passkeysPda);
      const nonceAfterFinalize = await actionNonceProcessed(w.walletPda);
      const snapshotAfterFinalize = await recoveryNonceSnapshotProcessed(w.walletPda);
      ...
      assert.deepEqual(
        {
          nonceAtFirst: nonceAtFirst.toString(),
          firstSnapshot: firstSnapshot.toString(),
          finalizeBump: Number(nonceAfterFinalize - nonceAtFirst),
          snapshotAfterFinalize: snapshotAfterFinalize.toString(),
          secondSnapshot: secondSnapshot.toString(),
        },
        {
          nonceAtFirst: "1",
  ...
  /// Stuurt de instructies als legacy-transactie; is die te groot (>1232),
  /// dan als v0 met een verse Address Lookup Table (zelfde aanpak als de
  /// maattest voor finalize_unfreeze + remove_passkey hierboven).
  async function sendMaybeWithAlt(ixs: TransactionInstruction[], lookupAddresses: PublicKey[]) {
    const payer = payerKeypair();
    const legacyMsg = new TransactionMessage({
      payerKey: payer.publicKey,
      recentBlockhash: (await provider.connection.getLatestBlockhash()).blockhash,
    ...
    const legacyTx = new VersionedTransaction(legacyMsg);
    legacyTx.sign([payer]);
    if (legacyTx.serialize().length <= 1232) {
      return provider.sendAndConfirm(new anchor.web3.Transaction().add(...ixs));
    }
    const recentSlot = await provider.connection.getSlot("finalized");
    const [createAltIx, altAddress] = AddressLookupTableProgram.createLookupTable({
    ...
    vtx.sign([payer]);
    assert.isAtMost(vtx.serialize().length, 1232, "ook met ALT te groot");
    const sig = await provider.connection.sendTransaction(vtx);
    const result = await provider.connection.confirmTransaction(sig, "processed");
    if (result.value.err) throw new Error(`transactie faalde: ${JSON.stringify(result.value.err)}`);
    return sig;
  }

  /// [secp256r1, initiate_unfreeze] op een opgegeven nonce (voor gebruik in
  /// een transactie ná een andere nonce-verhogende instructie).
  async function buildInitiateUnfreezeIxs(
    signingPasskey: TestPasskey,
    walletPda: PublicKey,
    pendingActionPda: PublicKey,
    passkeysPda: PublicKey,
    ...
    const ix = await program.methods
      ...
      .instruction();
    return [secp256r1Ix, ix];
  }

  /// [secp256r1, finalize_unfreeze] met een vooraf bekende commitment, zodat
  /// de aanval ook op te bouwen is als de PendingAction al gesloten is.
  async function buildFinalizeUnfreezeIxsWithCommitment(
    signingPasskey: TestPasskey,
    walletPda: PublicKey,
    pendingActionPda: PublicKey,
    passkeysPda: PublicKey,
  ...
  async function accountExistsProcessed(address: PublicKey): Promise<boolean> {
    return (await provider.connection.getAccountInfo(address, "processed")) !== null;
  }

  /// Anchor-framework-fout (geen programmafout, staat niet in de IDL).
  const ACCOUNT_NOT_INITIALIZED = "AccountNotInitialized (3012)";

  describe("Recovery: initiate_recovery sluit een wachtende PendingAction; zolang een recovery loopt is de wachtrij leeg (sectie 160)", () => {
    // De exacte aanvalsvolgorde uit de review van sectie 159 (M-1): de
    // eigenaar heeft alleen nog de backup-sleutel, de dief heeft de enige
    // passkey.
    it("[160] M-1: bevriezen, initiate_unfreeze door de dief, initiate_recovery, 24u: de dief kan niet annuleren + ontdooien + uitgeven (de wachtende Unfreeze is bij initiate_recovery gesloten)", async () => {
      const w = await createWallet();
      const thief = w.passkey;

      await callFreezeViaBackup(w.backupAuthority, w.walletPda);
      await callInitiateUnfreeze(thief, w.walletPda, w.pendingActionPda, w.passkeysPda);
      const pending = await program.account.pendingAction.fetch(w.pendingActionPda, "processed");
      const commitment = Buffer.from(pending.actionCommitment);
      const pendingRent = await lamportsProcessed(w.pendingActionPda);
      const backupBefore = await lamportsProcessed(w.backupAuthority.publicKey);

      const initiate = await outcomeOf(callInitiateRecovery(w.backupAuthority, w.walletPda, dummyNewOwnerPasskey()));
      const pendingClosed = !(await accountExistsProcessed(w.pendingActionPda));
      const backupGain = (await lamportsProcessed(w.backupAuthority.publicKey)) - backupBefore;
      ...
      // Transactie 1 van de dief: annuleren + ontdooien (cancel verhoogt de nonce met 1).
      const nonce = await actionNonceProcessed(w.walletPda);
      const cancelIxs = await buildCancelRecoveryIxs(thief, w.walletPda, w.passkeysPda);
      const finalizeIxs = await buildFinalizeUnfreezeIxsWithCommitment(
        thief,
        w.walletPda,
        w.pendingActionPda,
        w.passkeysPda,
      ...
      const cancelAndUnfreeze = await outcomeOf(
        sendMaybeWithAlt([...cancelIxs, ...finalizeIxs], [w.walletPda, w.passkeysPda, w.pendingActionPda])
      );

      // Transactie 2 van de dief: uitgeven.
      const recipient = Keypair.generate().publicKey;
      const amount = new BN(100_000_000);
      const vaultBefore = await lamportsProcessed(w.vaultPda);
      const executeIxs = await buildExecuteIxs(
        thief,
        w.walletPda,
        w.vaultPda,
        w.passkeysPda,
        recipient,
        amount,
        await actionNonceProcessed(w.walletPda)
      );
      const spend = await outcomeOf(provider.sendAndConfirm(new anchor.web3.Transaction().add(...executeIxs)));
      const wallet = await program.account.walletAccount.fetch(w.walletPda, "processed");
      ...
      assert.deepEqual(
        {
          initiate,
          pendingClosed,
          rentToBackup: backupGain === pendingRent,
          cancelAndUnfreeze,
          spend,
          vaultUnchanged: (await lamportsProcessed(w.vaultPda)) === vaultBefore,
          recipientBalance: await lamportsProcessed(recipient),
          recoveryStillActive: wallet.recoveryState !== null,
          disarmed: wallet.disarmed,
        },
        {
          ...
          pendingClosed: true,
          rentToBackup: true,
          cancelAndUnfreeze: ACCOUNT_NOT_INITIALIZED,
          spend: expectedError("RecoveryAlreadyInProgress"),
          vaultUnchanged: true,
          recipientBalance: 0,
          recoveryStillActive: true,
          disarmed: true,
    ...
    it("[160] initiate_recovery sluit elke soort wachtende actie (withdrawal, token transfer, advanced action, via sessie, drempelwijziging, unfreeze); rent naar de backup authority, nonce ongewijzigd", async () => {
      type Setup = { name: string; make: () => Promise<Awaited<ReturnType<typeof createWallet>>> };
      const setups: Setup[] = [
        {
          name: "withdrawal",
          make: async () => {
            ...
            await callInitiateWithdrawal(
              ...
              new BN(anchor.web3.LAMPORTS_PER_SOL / 2)
            );
            return w;
          },
        },
        ...
        {
          name: "advanced action via sessie",
          make: async () => setupSessionInitiatedAction(false),
        },
        {
          name: "drempelwijziging",
          make: async () => {
            const w = await createWallet();
            await callInitiateThresholdChange(
              w.passkey,
              w.walletPda,
              w.pendingActionPda,
              w.passkeysPda,
      ...
      for (const setup of setups) {
        const w = await setup.make();
        const kind = (await program.account.pendingAction.fetch(w.pendingActionPda, "processed")).kind;
        const pendingRent = await lamportsProcessed(w.pendingActionPda);
        const backupBefore = await lamportsProcessed(w.backupAuthority.publicKey);
        const nonceBefore = await actionNonceProcessed(w.walletPda);
        const initiate = await outcomeOf(callInitiateRecovery(w.backupAuthority, w.walletPda, dummyNewOwnerPasskey()));
        results[setup.name] = {
          kind,
          initiate,
          pendingClosed: !(await accountExistsProcessed(w.pendingActionPda)),
          rentToBackup: (await lamportsProcessed(w.backupAuthority.publicKey)) - backupBefore === pendingRent,
          nonceUnchanged: (await actionNonceProcessed(w.walletPda)) === nonceBefore,
          snapshotIsNonce: (await recoveryNonceSnapshotProcessed(w.walletPda)) === nonceBefore,
        };
        expected[setup.name] = {
          kind,
          initiate: "ok",
          ...
          rentToBackup: true,
          nonceUnchanged: true,
          snapshotIsNonce: true,
        };
      }
      // Alle vijf de soorten moeten langsgekomen zijn (0-4; via sessie is ook 2).
      const kinds = new Set(Object.values(results).map((r: any) => r.kind));
      assert.deepEqual([...kinds].sort(), [0, 1, 2, 3, 4]);
      assert.deepEqual(results, expected);
    });

    it("[160] initiate_recovery zonder wachtende actie werkt gewoon: het pending_action-adres blijft leeg, de backup authority ontvangt niets", async () => {
      const w = await createWallet();
      const existedBefore = await accountExistsProcessed(w.pendingActionPda);
      const backupBefore = await lamportsProcessed(w.backupAuthority.publicKey);
      const nonceBefore = await actionNonceProcessed(w.walletPda);
      const initiate = await outcomeOf(callInitiateRecovery(w.backupAuthority, w.walletPda, dummyNewOwnerPasskey()));
      const wallet = await program.account.walletAccount.fetch(w.walletPda, "processed");
      ...
      assert.deepEqual(
        {
          existedBefore,
          initiate,
          existsAfter: await accountExistsProcessed(w.pendingActionPda),
          backupGain: (await lamportsProcessed(w.backupAuthority.publicKey)) - backupBefore,
          recoveryActive: wallet.recoveryState !== null,
          nonceUnchanged: (await actionNonceProcessed(w.walletPda)) === nonceBefore,
        },
        {
          existedBefore: false,
    ...
    it("[160] tijdens een recovery kan geen nieuwe wachtende actie ontstaan: alle zes initiërende instructies weigeren (RecoveryAlreadyInProgress), het pending_action-adres blijft leeg", async () => {
      // Wallet met toegestaan programma, sessiesleutel en een al wachtende
      // sessie-actie; initiate_recovery sluit die.
      const w = await setupSessionInitiatedAction(false);
      const { mint, vaultTokenAccount, recipientTokenAccount } = await setupMintAndAccounts(
        w.vaultPda,
        provider.wallet.publicKey,
        1_000
      ...
      await callInitiateRecovery(w.backupAuthority, w.walletPda, dummyNewOwnerPasskey());
      const closedByRecovery = !(await accountExistsProcessed(w.pendingActionPda));
      // Zonder dit faalt elke poging hieronder al op "account already in use".
      assert.isTrue(closedByRecovery, "initiate_recovery had de wachtende sessie-actie moeten sluiten");

      const attempts: Record<string, string> = {};
      attempts.withdrawal = await outcomeOf(
        callInitiateWithdrawal(
          w.passkey,
          w.walletPda,
          w.pendingActionPda,
          w.passkeysPda,
      ...
      attempts.tokenTransfer = await outcomeOf(
        callInitiateTokenTransfer(
          w.passkey,
          w.walletPda,
          w.pendingActionPda,
      ...
      const fixture1 = await setupAssignCpiFixture();
      attempts.advancedAction = await outcomeOf(
        callInitiateAdvancedAction(
          w.passkey,
          w.walletPda,
          w.vaultPda,
          ...
          w.passkeysPda,
          SystemProgram.programId,
          [{ pubkey: fixture1.target.publicKey, isWritable: true, isSigner: true }],
          fixture1.assignIx.data,
          [fixture1.target]
        )
      );
      const fixture2 = await setupAssignCpiFixture();
      attempts.advancedActionViaSession = await outcomeOf(
        callInitiateAdvancedActionViaSession(
          w.sessionKeypair,
          w.walletPda,
          w.vaultPda,
          ...
          w.passkeysPda,
          SystemProgram.programId,
          [{ pubkey: fixture2.target.publicKey, isWritable: true, isSigner: true }],
          fixture2.assignIx.data,
          [fixture2.target]
        )
      );
      attempts.thresholdChange = await outcomeOf(
        callInitiateThresholdChange(
          w.passkey,
          w.walletPda,
          w.pendingActionPda,
          w.passkeysPda,
      ...
      // initiate_unfreeze vereist een bevroren wallet; bevriezen via de
      // backup authority mag tijdens een recovery (sectie 159).
      await callFreezeViaBackup(w.backupAuthority, w.walletPda);
      attempts.unfreeze = await outcomeOf(
        callInitiateUnfreeze(w.passkey, w.walletPda, w.pendingActionPda, w.passkeysPda)
      );

      const blocked = expectedError("RecoveryAlreadyInProgress");
      assert.deepEqual(
        { closedByRecovery, attempts, existsAfter: await accountExistsProcessed(w.pendingActionPda) },
        {
          closedByRecovery: true,
          attempts: {
            withdrawal: blocked,
            tokenTransfer: blocked,
            advancedAction: blocked,
            advancedActionViaSession: blocked,
            thresholdChange: blocked,
            unfreeze: blocked,
          },
          existsAfter: false,
        }
    ...
    // De patstelling zoals gedocumenteerd in STATUS.md sectie 160: de dief
    // kan annuleren en in dezelfde transactie een Unfreeze klaarzetten; de
    // eigenaar heeft dan 24u om met een nieuwe initiate_recovery die weer te
    // sluiten. Reageert de eigenaar niet, dan ontdooit de dief na 24u -
    // dat is de bekende, geaccepteerde grens (een backup-sleutel wint het
    // nooit van een passkey die het veto heeft).
    it("[160] patstelling: annuleren + Unfreeze klaarzetten in één transactie slaagt; een nieuwe initiate_recovery sluit die weer en de dief kan na 24u niet ontdooien; zonder reactie van de eigenaar ontdooit de dief wel", async () => {
      const w = await createWallet();
      const thief = w.passkey;
      const lookup = [w.walletPda, w.passkeysPda, w.pendingActionPda, SystemProgram.programId];

      await callFreezeViaBackup(w.backupAuthority, w.walletPda);
      await callInitiateRecovery(w.backupAuthority, w.walletPda, dummyNewOwnerPasskey());

      // Ronde 1: de dief annuleert en zet een Unfreeze klaar.
      const nonce1 = await actionNonceProcessed(w.walletPda);
      const round1 = await outcomeOf(
        sendMaybeWithAlt(
          [
            ...(await buildCancelRecoveryIxs(thief, w.walletPda, w.passkeysPda)),
            ...(await buildInitiateUnfreezeIxs(thief, w.walletPda, w.pendingActionPda, w.passkeysPda, nonce1 + 1n)),
          ],
          lookup
        )
      );
      const pending1 = await program.account.pendingAction.fetch(w.pendingActionPda, "processed");
      const commitment = Buffer.from(pending1.actionCommitment);

      // De eigenaar reageert binnen 24u met een nieuwe recovery.
      const ownerResponse = await outcomeOf(callInitiateRecovery(w.backupAuthority, w.walletPda, dummyNewOwnerPasskey()));
      const closedByOwner = !(await accountExistsProcessed(w.pendingActionPda));

      await advanceOnChainClockPast(
        provider.connection,
        payerKeypair(),
        pending1.timelockStartedAt.toNumber() + FAST_TIMELOCK_SECONDS
      );
      const nonce2 = await actionNonceProcessed(w.walletPda);
      const thiefAfter24h = await outcomeOf(
        sendMaybeWithAlt(
          [
            ...(await buildCancelRecoveryIxs(thief, w.walletPda, w.passkeysPda)),
            ...(await buildFinalizeUnfreezeIxsWithCommitment(
              thief,
              w.walletPda,
              w.pendingActionPda,
              w.passkeysPda,
      ...
      const disarmedAfterRound1 = (await program.account.walletAccount.fetch(w.walletPda, "processed")).disarmed;

      // Ronde 2: de dief herhaalt; de eigenaar reageert nu niet. (Won de
      // dief ronde 1 al, dan loopt er geen recovery meer om te annuleren.)
      let round2 = "geen lopende recovery";
      let unanswered = "niet uitgevoerd";
      if ((await program.account.walletAccount.fetch(w.walletPda, "processed")).recoveryState !== null) {
        const nonce3 = await actionNonceProcessed(w.walletPda);
        round2 = await outcomeOf(
          sendMaybeWithAlt(
            [
              ...(await buildCancelRecoveryIxs(thief, w.walletPda, w.passkeysPda)),
              ...(await buildInitiateUnfreezeIxs(thief, w.walletPda, w.pendingActionPda, w.passkeysPda, nonce3 + 1n)),
            ],
            lookup
          )
        );
        const pending2 = await program.account.pendingAction.fetch(w.pendingActionPda, "processed");
        await advanceOnChainClockPast(
          provider.connection,
          payerKeypair(),
          pending2.timelockStartedAt.toNumber() + FAST_TIMELOCK_SECONDS
        );
        unanswered = await outcomeOf(callFinalizeUnfreeze(thief, w.walletPda, w.pendingActionPda, w.passkeysPda));
      }
      const wallet = await program.account.walletAccount.fetch(w.walletPda, "processed");
      ...
      assert.deepEqual(
        {
          ...
          round2,
          unanswered,
          disarmedAtEnd: wallet.disarmed,
          recoveryAtEnd: wallet.recoveryState !== null,
        },
        {
          round1: "ok",
          ownerResponse: "ok",
          closedByOwner: true,
          thiefAfter24h: ACCOUNT_NOT_INITIALIZED,
          disarmedAfterRound1: true,
          round2: "ok",
          unanswered: "ok",
          disarmedAtEnd: false,
          recoveryAtEnd: false,
